...
def config_prime(config_file, prime_file, tree_file, num_sites, nthreads, smooth):
    """
    Load the input config file, and generate a config file for cross-validation.
    """
    # Load the input config file and copy the content to the new config file
    ...
    with open(prime_file, 'w') as dst:
        ...
        dst.write(f"\n")
        dst.write(f"# output file name\n")
        dst.write(f"primeoutfile = treepl_prime_results.txt\n")
        dst.write(f"\n")
        dst.write(f"# the number of threads\n")
        dst.write(f"nthreads = {nthreads}\n")
        dst.write(f"\n")
...
def run_treepl(config_file):
    ...
    try:
        ...
        print(f"[ERROR] Fail to run TreePL: {e.stderr}")
        raise
...
def main():
    ...
    print("\n[STEP 2/7] Running cross-validation (this may take some time)...")
    try:
        run_treepl(temp_cv_config)
    ...
    temp_cv_results = "treepl_cv_results.txt"
    ...
    print(f"\n[STEP 5/7] Running TreePL prime... (This may take some time, using {args.nthreads} threads)")
    result = subprocess.run(
        ['treePL', 'treepl_prime.config'], 
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True
    )
